                    ...
                    <div class="card-header row gutters-5">
                        ...
                        <div class="col-md-4">
                            <input type="text" class="form-control" id="sort_city" name="sort_city"
                                ...
                                placeholder="{{ translate('Type district name & Enter') }}">
                        </div>
                                ...
                                <select class="form-control aiz-selectpicker" data-live-search="true" id="sort_state"
                                    ...
                                    @foreach ($states as $state)
                                        <option value="{{ $state->id }}"
                                            @if ($sort_state == $state->id) selected @endif {{ $sort_state }}>
                                            {{ $state->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control aiz-selectpicker" data-live-search="true" id="sort_country"
                                    ...
                                    @foreach ($countries as $country)
                                        <option value="{{ $country->id }}"
                                            @if ($sort_country == $country->id) selected @endif {{ $sort_country }}>
                                            {{ $country->name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <table class="table aiz-table mb-0">
                        <thead>
                            <tr>
                                ...
                                <th data-breakpoints="" class="text-right">{{ translate('Options') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <label class="aiz-switch aiz-switch-success mb-0">
                                            <input onchange="triggerConfirmation(this)" value="{{ $city->id }}"
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('cities.store') }}" method="POST">
                        ...
                        <div class="form-group mb-3">
                            <label for="district_code">{{ translate('District Code') }}</label>
                            <input type="text" name="district_code" class="form-control" placeholder="CG01"
                                pattern="^CG[0-9]{2}$" title="Format must be CG followed by 2 digits (Example: CG01)"
...
@section('script')
    <script type="text/javascript">
        ...
        function triggerConfirmation(el) {
            ...
            if (isTurningOn) {
                ...
                $('#confirm-modal .modal-body p').text(
                    `If you disable a City, all associated Areas under that city will also be automatically disabled.`);
        ...
        document.querySelector('input[name="district_code"]').addEventListener('input', function() {
