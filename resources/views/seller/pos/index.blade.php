...
@php
    $indiaCountry = \App\Models\Country::where('status', 1)->where('name', 'India')->first();
@endphp
            ...
            <div class="modal-content">
                ...
                <form class="form-horizontal" action="{{ route('addresses.store') }}" method="POST"
                    ...
                    <div class="modal-body">
                        ...
                        <input type="hidden" name="country_id" value="{{ optional($indiaCountry)->id }}"
                            data-default-country="india">
                        <div class="form-group">
                            <div class="row">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('#product-list').on('click', '.add-plus:not(.c-not-allowed)', function() {
                ...
                $.post('{{ route('seller.pos.addToCart') }}', {
        ...
        $("#confirm-address").click(function() {
            ...
            $.ajax({
                ...
                success: function(data, textStatus, jqXHR) {
                    AIZ.plugins.notify('success', data.message || '{{ translate('Shipping address saved successfully.') }}');
                    $('#new-customer').modal('hide');
                },
                error: function() {
                    AIZ.plugins.notify('danger', '{{ translate('Please fill all required fields correctly before saving.') }}');
                }
            })
        });
...
>>>>>>> Stashed changes
                                    ...
                                    <span class="badge badge-inline badge-dark fs-12">
                                        ${data.data[i].stock_qty}
                                    </span>
                        ...
                        ${data.data[i].variant && data.data[i].variant.trim() !== ''

                            ? `<span class="badge badge-inline badge-warning absolute-bottom-left mb-1 ml-1 mr-0 fs-13 text-truncate">
                                        ${data.data[i].variant}
                                      </span>`
            ...
            if (data.links.next != null) {
                $('#load-more').find('.btn')
                    .html('{{ translate('Load More.') }}');
            } else {
                $('#load-more').find('.btn')
                    .html('{{ translate('Nothing more found.') }}');
            }
        }

        function removeFromCart(id) {
            $.post('{{ route('seller.pos.removeFromCart') }}', {
                _token: AIZ.data.csrf,
                id: id
            }, function(data) {
                updateCart(data);
        ...
        function setDiscount() {
            ...
            $.post('{{ route('seller.pos.setDiscount') }}', {
                _token: AIZ.data.csrf,
                discount: discount
            }, function(data) {
                updateCart(data);
        ...
        function setShipping() {
            ...
            $.post('{{ route('seller.pos.setShipping') }}', {
                _token: AIZ.data.csrf,
                shipping: shipping
            }, function(data) {
                updateCart(data);
        ...
        function getShippingAddress() {
            $.post('{{ route('seller.pos.getShippingAddress') }}', {
                _token: AIZ.data.csrf,
                id: $('select[name=user_id]').val()
            }, function(data) {
                $('#shipping_address').html(data);
                initializeIndiaAddressForms($('#shipping_address'));
            });
        }

        function updateSessionUserCartData() {
            $.post('{{ route('seller.pos.updateSessionUserCartData') }}', {
                _token: AIZ.data.csrf,
        ...
        function add_new_address() {
            ...
            initializeIndiaAddressForms($('#new-address-modal'));
            $("#close-button").click();
        }

        function initializeIndiaAddressForms(container) {
            container.find('[name=country_id][data-default-country="india"]').each(function() {
        ...
        function orderConfirmation() {
            ...
            $.post('{{ route('seller.pos.getOrderSummary') }}', {
                _token: AIZ.data.csrf
            }, function(data) {
        ...
        function submitOrder(payment_type) {
            ...
            $.post('{{ route('seller.pos.order_place') }}', {
                ...
                if (data.success == 1) {
                    ...
                    AIZ.plugins.notify('danger', data.message || '{{ translate('Something went wrong while placing the order.') }}');
                }
            });
        }
